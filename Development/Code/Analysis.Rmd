...
for (functional.form in functional.forms){
  ...
  model <- rms::lrm(form, data=single.imputation) # TODO: SINGLE IMPUTATION
...
best.flexible.model <- rms::lrm(HypoP ~ rms::rcs(dPTH, 4) + CorrCa24u + BSKgezien + Age_Years + Sex + surgery_type + CHKD,
                                ...
                                maxit=1000, x=TRUE, y=TRUE)
lrtest(best.flexible.model, full.model.single) # TODO: mention single imputation
```
...
table(single.imputation$HypoP, rowMeans(dPTH)>0.7)
...
# hypoP table
table(single.imputation$HypoP, str.pred)
round(table(single.imputation$HypoP, str.pred)/rbind(table(str.pred), table(str.pred))*100, 1)
