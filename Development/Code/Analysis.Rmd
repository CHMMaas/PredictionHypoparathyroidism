...
OR.table <- data.frame(names=names.coef,
                                coef.full=sprintf("%.3f", OR.full.model[names.coef]),
                                CI.full=c(paste0("[", sprintf("%.3f", CI.full.model[names.coef, 1]), "; ", 
                                               sprintf("%.3f", CI.full.model[names.coef, 2]), "]")),
...
for (i in 1:m){
  ...
  high.dPTH <- cbind(high.dPTH, ifelse(selected.imputation$dPTH>70, 1, 0))
...
table(single.imputation$Readmission, rowMeans(high.dPTH)>0.5)
