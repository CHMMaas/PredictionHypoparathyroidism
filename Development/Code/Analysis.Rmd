...
for (type in c("full", "final")){
  for (j in 1:4){
    ...
    y.hypop.j <- as.numeric(work.data$HypoP[work.data$Validatie1==Centers[j]])-1
    y.read.j <- as.numeric(work.data$Readmission[work.data$Validatie1==Centers[j]])-1
    y.hypop.notj <- as.numeric(work.data$HypoP[work.data$Validatie1!=Centers[j]])-1
    ...
    for (i in 1:m){
      ...
      f.notj <- rms::lrm(Readmission~lp.notj,
      ...
      predict.df <- as.matrix(cbind(rep(1, nrow(data.i[work.data$Validatie1==Centers[j],])),
                                data.i[work.data$Validatie1==Centers[j], c("dPTH", "CorrCa24u")],
                                as.numeric(data.i[work.data$Validatie1==Centers[j], "BSKgezien"])-1))
...
for (type in c("full", "final")){
  png(file=paste0(file.path, "Results/leave.one.out.cross.validation.hypop.", type, ".png"), width=16, height=16, units="cm", res=300)
  par(mar=rep(0, 4))
  ...
  png(file=paste0(file.path, "Results/leave.one.out.cross.validation.readmission.", type, ".png"), width=16, height=16, units="cm", res=300)
  par(mar=rep(0, 4))
  layout(matrix(1:4, ncol=2, byrow=TRUE))
  Centers.val <- rep(1:4, 3)
  for (i in 1:4){
...
png(file=paste0(file.path, "Results/model.performance.calibrated.model.png"), 
    width=400, height=400, units="px")
...
# Save Table
```{r, eval=TRUE, warning=FALSE}
names.coef <- c("Intercept", "dPTH", "CorrCa24u", # "dCorrCa",
                "BSKgezien - No:Yes",
...
Cindex.table <- c("C-index optimism-corrected",
                  ...
                  paste0(sprintf("%.2f", out.calibrated$cindex-optimism.C), 
                         " [", sprintf("%.2f", out.final$cindex.lower-optimism.C), "; ", 
                         sprintf("%.2f", out.final$cindex.upper-optimism.C), "]"))
