...
optimism.C.read<-v.readmission["Dxy","optimism"]/2          # optimism C-index, /2 because  C.orig - C.B the +1 cancels out
...
for (i in 1:m){
  ...
  lp.refitted.full <- cbind(lp.refitted.full, predict(refitted.full.model,
                                              ...
                                              type="lp"))
  lp.refitted.final.i <- predict(refitted.final.model,
                                                newdata=imputed.data.i,
                                                type="lp")
  lp.refitted.final <- cbind(lp.refitted.final, lp.refitted.final.i)

  predict.df <- as.matrix(cbind(rep(1, nrow(imputed.data.i)),
                                imputed.data.i[, c("dPTH", "CorrCa24u")],
                                as.numeric(imputed.data.i[, "BSKgezien"])-1))
  
  lp.shrunk.i <- shrinkage.factor.read*lp.refitted.final.i
  fit.extra.refitted.i <- lrm.fit(y=imputed.data.i$HypoP,
                                  offset=lp.shrunk.i) # TODO: for each imputation?
  intercept.extra.refitted.i <- coef(fit.extra.refitted.i)[1]
  shrunk.intercept.refitted.final.i <- shrinkage.factor.read*refitted.final.model$coefficients["Intercept"]+intercept.extra.refitted.i

  coef.refitted.final.i <- c(shrinkage.factor.read*coef(refitted.final.model)[1]+shrunk.intercept.refitted.final.i,
                             shrinkage.factor.read*coef(refitted.final.model)[-1])
  lp.refitted.shrunk.final <- cbind(lp.refitted.shrunk.final, as.numeric(t(coef.refitted.final.i) %*% t(predict.df)))
...
for (model.type in model.types){
  ...
  # don't apply shrinkage to refitted coefficients
  # shrink.fact <- shrinkage.factor.read
...
calibrated.readmission.OR <- exp(c(cal.fact.final*shrunk.intercept.final+mean(cal.int),
                                   cal.fact.final*shrunk.coef.final.model))
...
Cindex.table <- c("C-index",
                  sprintf("%.2f", out.refitted.full$cindex-optimism.C.read), 
                  paste0("[", sprintf("%.2f", out.refitted.full$cindex.lower-optimism.C.read), "; ", 
                         sprintf("%.2f", out.refitted.full$cindex.upper-optimism.C.read), "]"),
                  "",
                  sprintf("%.2f", out.refitted.final$cindex-optimism.C.read), 
                  paste0("[", sprintf("%.2f", out.refitted.final$cindex.lower-optimism.C.read), "; ", 
                         sprintf("%.2f", out.refitted.final$cindex.upper-optimism.C.read), "]"),
                  "")
final.table <- rbind(OR.table, Cindex.table)
openxlsx::write.xlsx(final.table,
            rowNames=FALSE,
...
coef.readmission.model.webapp.not.shrunk <- c(cal.fact.final*final.model$coefficients["Intercept"]+mean(cal.int),
                                   cal.fact.final*summary(final.model, 
                                                           dPTH=c(1, 2), 
                                                           CorrCa24u=c(1, 2), 
                                                           BSKgezien="No")[which(summary(final.model)[, "Type"]==1), 
                                                                           c("Effect")])
coef.readmission.model.webapp <- c(cal.fact.final*shrunk.intercept.final+mean(cal.int),
                                   cal.fact.final*coef.hypop.model.webapp[-1])
...
for (i in 1:m){
  ...
  lp.read.not.shrunk <- cbind(lp.read.not.shrunk,
                  coef.readmission.model.webapp.not.shrunk["Intercept"]+
                  coef.readmission.model.webapp.not.shrunk["dPTH"]*selected.imputation$dPTH+
                  coef.readmission.model.webapp.not.shrunk["CorrCa24u"]*selected.imputation$CorrCa24u+
                  coef.readmission.model.webapp.not.shrunk["BSKgezien - Yes:No"]*ifelse(selected.imputation$BSKgezien == "Yes", 1, 0))
}
...
```
```{r, eval=TRUE}
PredictionTools::val.prob.mi(lp.mi=lp.hypop, y=as.numeric(work.data$HypoP)-1, dist=TRUE)                       # includes shrinakge
PredictionTools::val.prob.mi(lp.mi=lp.read, y=as.numeric(work.data$Readmission)-1, dist=TRUE)                  # includes shrinkage and calibration
PredictionTools::val.prob.mi(lp.mi=lp.read.not.shrunk, y=as.numeric(work.data$Readmission)-1, dist=TRUE)       # includes only calibration, no shrinkage
PredictionTools::val.prob.mi(lp.mi=lp.refitted.final, y=as.numeric(work.data$Readmission)-1, dist=TRUE)        # no shrinkage
PredictionTools::val.prob.mi(lp.mi=lp.refitted.shrunk.final, y=as.numeric(work.data$Readmission)-1, dist=TRUE) # includes shrinkage
```
