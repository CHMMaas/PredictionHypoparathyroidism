...
for (model.type in model.types){
  ...
  lp.shrunk <- shrinkage.factor.hypop*lp
  fit.extra <- lrm.fit(y=work.data$HypoP,
  ...
  if (model.type=="simple"){
    ...
    coef.model <- shrinkage.factor.hypop*summary.model["Effect"]
  ...
  CI.df <- exp(rbind(c(shrinkage.factor.hypop*Int.CI.lower+intercept.extra,
                       shrinkage.factor.hypop*Int.CI.upper+intercept.extra),
                     shrinkage.factor.hypop*CI.coef))
  if (model.type!="simple"){
...
Centers.title <- paste("Cohort", 1:4)
for (type in c("full", "final")){
  for (j in 1:4){
    ...
    # shrinkage.factor.hypop.notj <- v["Slope","test"]
    
    # cat("Now calculating for center", Centers[j], "\n")
    # print(model.notj$coefficients)
    # print(shrinkage.factor.hypop.notj)
    ...
    out.j <- PredictionTools::val.prob.mi(lp.mi=lp.j, y=y.j, main=Centers.title[j], g=3, dist=TRUE)
    grDevices::dev.off()
  }
}
...
```
# Select model through backward selection for readmission model
```{r, eval=TRUE}
form.redeveloped.model <- Readmission ~ dPTH + 
  CorrCa24u  
  # BSKgezien + 
  # Age_Years + 
  # Sex 
  # surgery_type
  # CHKD
redeveloped.model <- Hmisc::fit.mult.impute(form.redeveloped.model, lrm, 
                                          xtrans=imputed.data,
                                          data=work.data,
                                          n.impute=m, pr=FALSE, fit.reps=TRUE, 
                                          fitargs=list(y=TRUE, x=TRUE, se.fit=TRUE))
anova(redeveloped.model)
```
# Shrinkage factor for readmission model
```{r, eval=TRUE}
v.readmission <- rms::validate(refitted.full.model, method="boot", bw=TRUE, rule="p", sls=0.05, B=1000,
                   pr=FALSE, type="individual")
# in each bootstrap BSKgezien is omitted, only dPTH and CorrCa24u remain
shrinkage.factor.read <- v.readmission["Slope", "test"]
cat("Shrinkage factor     :", shrinkage.factor.read, "\n")
# similar shrinkage factor
```
...
refitted.final.model <- Hmisc::fit.mult.impute(form.refitted.final.model, lrm, 
                                          ...
                                          fitargs=list(y=TRUE, x=TRUE, se.fit=TRUE))
```
# Test refitted versus redeveloped model
```{r, eval=TRUE}
# final model and refitted model have similar coefficients
coef(final.model)
coef(refitted.final.model)

# BSKgezien seems to not be relevant for hypocalcemia-related readmissions
coef(redeveloped.model)
AIC(refitted.final.model)
AIC(redeveloped.model)

# model without BSKgezien is better, but not significantly
lrtest(refitted.final.model, redeveloped.model)
```
...
cal.fact.final <- cal.fact.combined$est
print(paste0(sprintf("%.2f", cal.fact.final), " [",
            sprintf("%.2f", cal.fact.final+qnorm(.025)*cal.fact.combined$se), "; ",
            sprintf("%.2f", cal.fact.final+qnorm(.975)*cal.fact.combined$se), "]"))
```
# Calibratino plots of readmission models
```{r, eval=TRUE}
lp.refitted.full <- c()
lp.refitted.final <- c()
lp.redeveloped <- c()
for (i in 1:m){
  lp.refitted.full <- predict(refitted.full.model,
                  newdata=mice::complete(imputed.data, i), 
                  type="lp")
  lp.refitted.final <- predict(refitted.final.model,
                  newdata=mice::complete(imputed.data, i), 
                  type="lp")
  lp.redeveloped <- predict(redeveloped.model,
                  newdata=mice::complete(imputed.data, i), 
                  type="lp")
  lp.calibrated <- cal.fact.final*predict(final.model,
                                          newdata=mice::complete(imputed.data, i), 
                                          type="lp")
}

model.types <- c("refitted.full", "refitted.final", "refitted.final.shrunk", 
                 "calibrated", "redeveloped", "redeveloped.shrunk")
for (model.type in model.types){
  if (model.type=="refitted.final.shrunk"){
    shrink.fact.cal.plot <- shrinkage.factor.hypop
    lp.model <- lp.refitted.final
  } else if (model.type=="redeveloped.shrunk"){
    shrink.fact.cal.plot <- shrinkage.factor.read
    lp.model <- lp.redeveloped
  } else{
    shrink.fact.cal.plot <- 1
    lp.model <- eval(parse(text=paste0("lp.", model.type)))
  }
  png(file=paste0(file.path, "Results/model.performance.", model.type, ".model.png"), 
    width=400, height=400, units="px")
  cal.plot <- PredictionTools::val.prob.mi(lp.mi=shrink.fact.cal.plot*lp.model, dist=TRUE, 
...
# Save results
```{r, eval=TRUE}
model.types <- c("refitted.full", "refitted.final", "redeveloped", "calibrated")
for (model.type in model.types){
  lp.model <- eval(parse(text=paste0("lp.", model.type)))
  if (model.type=="calibrated"){
    fitted.model <- final.model
  } else{
    fitted.model <- eval(parse(text=paste0(model.type, ".model")))
  }
  
  # TODO: shrinkage factor
  shrinkage.factor.readmission <- 1
  intercept.extra.readmission <- 0
  
  # save coefficients
  if (model.type=="redeveloped"){
    summary.model <- summary(fitted.model, 
                           dPTH=c(0, 1),
                           CorrCa24u=c(0.2, 0.1))[which(summary(fitted.model)[, "Type"]==1),]
  } else{
    summary.model <- summary(fitted.model, 
                           dPTH=c(0, 1),
                           CorrCa24u=c(0.2, 0.1),
  ...
  coef.model <- shrinkage.factor.readmission*summary.model[,  "Effect"]
  CI.coef <- summary.model[, c("Lower 0.95", "Upper 0.95")]
  if (model.type=="calibrated"){
    coef.model <- cal.fact.final*coef.model
    CI.coef <- cal.fact.final*CI.coef
    
    Int.CI.lower <- cal.fact.final*(fitted.model$coefficients["Intercept"]+qnorm(0.025)*sqrt(diag(fitted.model$var)[1]/nrow(work.data)))
    Int.CI.upper <- cal.fact.final*(fitted.model$coefficients["Intercept"]+qnorm(0.975)*sqrt(diag(fitted.model$var)[1]/nrow(work.data)))
  }
  assign(paste0("coef.", model.type, ".model"), coef.model)
  ...
  # save CI
  CI.df <- exp(rbind(c(shrinkage.factor.readmission*Int.CI.lower+intercept.extra.readmission,
                       shrinkage.factor.readmission*Int.CI.upper+intercept.extra.readmission),
                     shrinkage.factor.readmission*CI.coef))
  row.names(CI.df)[1] <- c("Intercept") # give intercept a name
  ...
  # # new intercept after shrinkage
  # lp <- predict(fitted.model)
  # lp.shrunk <- shrinkage.factor.readmission*lp
  # fit.extra <- lrm.fit(y=work.data$HypoP,
  #                       offset=lp.shrunk) # TODO: for each imputation?
  # intercept.extra.readmission <- coef(fit.extra)[1]
  # intercept.shrunk <- shrinkage.factor.readmission*fitted.model$coefficients["Intercept"]+intercept.extra.readmission
  # 
  # # check
  # mean(work.data$HypoP=="Yes") # event probability
  # mean(plogis(lp.shrunk+intercept.extra.readmission))  # same
  # 
  # # save results
  # assign(paste0(model.type, ".intercept.shrunk"), intercept.shrunk)
```
...
OR.table <- data.frame(names=names.coef,
                                ...
                                Chi.refitted.final=c("", sprintf("%.1f", an.refitted.final[-nrow(an.refitted.final), "Chi-Square"]),
                                             rep("", 4)),
                                coef.calibrated=c(sprintf("%.2f", OR.calibrated.model), rep("", 4)),
                                CI.calibrated=c(paste0("[", sprintf("%.2f", CI.calibrated.model[, 1]), "; ", 
                                               sprintf("%.2f", CI.calibrated.model[, 2]), "]"),
                                            rep("", 4)),
                                Chi.calibrated=c("", sprintf("%.1f", an.calibrated[-nrow(an.calibrated), "Chi-Square"]),
                                             rep("", 4)),
                                coef.redeveloped=c(sprintf("%.2f", OR.redeveloped.model), rep("", 5)),
                                CI.redeveloped=c(paste0("[", sprintf("%.2f", CI.redeveloped.model[, 1]), "; ", 
                                               sprintf("%.2f", CI.redeveloped.model[, 2]), "]"), rep("", 5)),
                                Chi.redeveloped=c("", sprintf("%.1f", an.redeveloped[-nrow(an.redeveloped), "Chi-Square"]),
                                            rep("", 5)))
Cindex.table <- c("C-index",
                  sprintf("%.2f", out.refitted.full$cindex), 
                  paste0("[", sprintf("%.2f", out.refitted.full$cindex.lower), "; ", sprintf("%.2f", out.refitted.full$cindex.upper), "]"),
                  "",
                  sprintf("%.2f", out.refitted.final$cindex), 
                  paste0("[", sprintf("%.2f", out.refitted.final$cindex.lower), "; ", sprintf("%.2f", out.refitted.final$cindex.upper), "]"),
                  "",
                  sprintf("%.2f", out.calibrated$cindex), 
                  paste0("[", sprintf("%.2f", out.calibrated$cindex.lower), "; ", sprintf("%.2f", out.calibrated$cindex.upper), "]"),
                  "",
                  sprintf("%.2f", out.redeveloped$cindex), 
                  paste0("[", sprintf("%.2f", out.redeveloped$cindex.lower), "; ", sprintf("%.2f", out.redeveloped$cindex.upper), "]"),
                  "")
final.table <- rbind(OR.table, Cindex.table)
openxlsx::write.xlsx(final.table,
            rowNames=FALSE,
...
# Save for webapp
```{r, eval=TRUE}
coef.hypop.model.webapp <- shrinkage.factor.hypop*summary(final.model, 
                                   dPTH=c(1, 2), 
                                   CorrCa24u=c(1, 2), 
                                   BSKgezien="No")[which(summary(final.model)[, "Type"]==1), 
                                                   c("Effect")]
coef.readmission.model.webapp <- summary(refitted.final.model, 
                                   dPTH=c(1, 2), 
                                   CorrCa24u=c(1, 2), 
                                   BSKgezien="No")[which(summary(refitted.final.model)[, "Type"]==1), 
                                                   c("Effect")]
intercept.readmission <- coef(refitted.final.model)[1]
var.cov.hypop <- vcov(final.model)
var.cov.readmission <- vcov(refitted.final.model)
save(shrinkage.factor.hypop, final.intercept.shrunk, intercept.readmission,
     coef.hypop.model.webapp, coef.readmission.model.webapp,
     var.cov.hypop, var.cov.readmission,
     single.imputation, refitted.final.model, redeveloped.model,
     imputed.data, work.data, shrinkage.factor.read,
...
table(work.data$HypoP, rowMeans(dPTH)>=70)
table(work.data$Readmission, rowMeans(dPTH)>=70)
...
table(str.pred)
round(table(str.pred)/nrow(work.data)*100, 1)

# hypoP table
table(work.data$HypoP, str.pred)
round(table(work.data$HypoP, str.pred)/rbind(table(str.pred), table(str.pred))*100, 1)

# readmission table
table(work.data$Readmission, str.pred)
round(table(work.data$Readmission, str.pred)/rbind(table(str.pred), table(str.pred))*100, 1)
```
# Compare predictions for HypoP and Readmission model
```{r, eval=TRUE}
plot(plogis(shrinkage.factor.hypop*predict(final.model, type="lp")), 
     plogis(shrinkage.factor.hypop*predict(refitted.final.model, type="lp")),
     xlab="Probability of Hypoparathyroidism", xlim=c(0, 1),
     ylab="Probability of Readmission", ylim=c(0, 1),
     main="Both shrunk with hypoP factor")
abline(0, 1)
plot(plogis(shrinkage.factor.hypop*predict(final.model, type="lp")), 
     plogis(shrinkage.factor.read*predict(redeveloped.model, type="lp")),
     xlab="Probability of Hypoparathyroidism", xlim=c(0, 1),
     ylab="Probability of Readmission", ylim=c(0, 1),
     main="Both shrunk and both redeveloped")
abline(0, 1)
```
