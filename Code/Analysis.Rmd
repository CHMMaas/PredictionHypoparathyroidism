...
dd<-rms::datadist(mice::complete(imputed.data, m.choose))
options(datadist='dd')
options(digits=8)
```
...
for (functional.form in functional.forms){
  ...
  model <- rms::lrm(form, data=mice::complete(imputed.data, m.choose)) # SINGLE IMPUTATION
...
form.flexible.model <- HypoP ~ rms::rcs(dPTH, 4) + 
  ...
  BSKgezien + 
  CHKD
flexible.model <- rms::lrm(form.flexible.model, data=mice::complete(imputed.data, m.choose), 
...
```{r, eval=TRUE}
full.model.single <- rms::lrm(form.full.model, data=mice::complete(imputed.data, m.choose), # SINGLE IMPUTATION
                              x=TRUE, y=TRUE)
best.flexible.model <- rms::lrm(HypoP ~ rms::rcs(dPTH, 4) + CorrCa24u + Age_Years + Sex + surgery_type + BSKgezien + CHKD,
                                data=mice::complete(imputed.data, m.choose), # SINGLE IMPUTATION
                                maxit=1000, x=TRUE, y=TRUE)
...
for (model.type in model.types){
  ...
  fit.extra <- lrm.fit(y=mice::complete(imputed.data, m.choose)$HypoP,
...
lp.full <- c()
for (i in 1:m){
  lp.simple <- cbind(lp.simple, predict(simple.model, newdata=mice::complete(imputed.data, i))) # TODO: apply shrinkage factor?a
...
single.imputed.data <- mice::complete(imputed.data, m.choose) # TODO: for each imputation?
for (type in c("full", "final")){
  for (j in 1:4){
    ...
    # fit on 3 centers
    model.notj <- rms::lrm(form.notj, data=single.imputed.data[work.data$Validatie1!=Centers[j],], x=TRUE, y=TRUE)
